...
# passing it on the cmdline makes the kernel call ksu_set_manager_appid(N) at boot,
...
#
# NOTE: hardcoding 10583 fails when the Manager's actual UID differs (e.g. 10561).
# We pin whatever the Manager's current UID is so the kernel always delivers the fd.
MANAGER_APPID=$(pm dump com.rifsxd.ksunext 2>/dev/null | grep "^    uid=" | sed 's/.*uid=//' | tr -d ' ')
if [ -z "$MANAGER_APPID" ]; then
  MANAGER_APPID=10583
  ui_print " ";
  ui_print " WARNING: Could not detect Manager UID, defaulting to 10583";
else
  ui_print " ";
  ui_print " Detected KSU Manager UID: $MANAGER_APPID";
fi
ui_print " Pre-registering KSU Manager appid (kernelsu.ksu_debug_manager_appid=$MANAGER_APPID)...";
...
if [ -f "$SPLITIMG/cmdline.txt" ]; then
  ...
  echo "$(cat "$SPLITIMG/cmdline.txt") kernelsu.ksu_debug_manager_appid=$MANAGER_APPID" > "$SPLITIMG/cmdline.txt"
  ...
  echo "$(cat "$CMDFILE") kernelsu.ksu_debug_manager_appid=$MANAGER_APPID" > "$CMDFILE"
  sed -i -e 's/^[ \t]*//' -e 's/  */ /g' -e 's/[ \t]*$//' "$CMDFILE"
  sed -i "s|^cmdline=.*|cmdline=$(cat "$CMDFILE")|" "$SPLITIMG/header"
  rm -f "$CMDFILE"
fi
ui_print " Manager appid pinned to $MANAGER_APPID via cmdline";

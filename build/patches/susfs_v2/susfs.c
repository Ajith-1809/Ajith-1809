...
int susfs_set_uname(struct st_susfs_uname* __user user_info) {
	...
	if (!strcmp(info.release, "default")) {
		/* Preserve the current spoof instead of capturing the live
		 * utsname()->release. Other actors (susfs4ksu module, KSU
		 * Manager / WebUI) call set_uname with "default" to re-assert
		 * their own view; re-reading the real utsname here would
		 * silently clobber an already-applied spoof at steady state.
		 * If nothing is spoofed yet, seed from the real utsname
		 * (benign first-time default). */
...
void susfs_spoof_uname(struct new_utsname* tmp) {
	/* Take the lock instead of bailing out when it is held. The stock
	 * spin_is_locked() early-return leaks the REAL utsname into tmp if a
	 * uname() reader races a concurrent set_uname (e.g. the susfs4ksu
	 * module's recurring `set_uname default` at post-fs-data/boot-completed).
	 * Deadlock-safe: no caller holds susfs_uname_spin_lock when calling here
	 * (susfs_set_uname releases before returning; compat + read hooks are
	 * lock-free), and this pairs with the plain spin_lock on the write side. */
	spin_lock(&susfs_uname_spin_lock);
	if (my_uname.release[0] != '\0') {

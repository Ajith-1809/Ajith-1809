# Release checklist — phoenix-v2.3 KSUNv3.0.0+SUSFSv1.5.9
...
- [ ] You have a GitHub account; the repo is private or public — GA will run on it.
...
- [ ] Open the GitHub repo's **Actions** tab → select **Build Phoenix KSUN v3.0.0 / SUSFS v1.5.9**.
- [ ] Click **Run workflow** → fill in `adb_serial` (your serial) → leave `upload_target` as `direct`.
- [ ] Watch the jobs: `host-prep` (~5 min) → `source-prep` (~3 min) → `build` (~45 min).
- [ ] When all three pass, click into the run → download these artifacts:
...
- [ ] Confirm `boot_backup_v2.2.img` exists locally (file size ≈ current boot size, ~65–90 MB).
...
- [ ] Confirm `ramdisk.cpio` exists and is non-empty (≥50 KB).
...
- [ ] Confirmed `SIGNED_BOOT` exists, size ≈ combined `Image.gz-dtb` + `ramdisk.cpio` (~70–100 MB).
...
- [ ] 10/10 PASS. If any FAIL → rollback (see §I).
...
- [ ] Upload `kernel-phoenix-v2.3-KSUNV3.0.0_SUSFSV1.5.9-YYYYMMDD.img` as a GitHub Release asset.

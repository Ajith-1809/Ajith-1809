...
/* SUSFS CMD dispatch */
/*********************/
/* These are called from our susfs_handle_sys_reboot supercall
 * in ksu_susfs_compat.c. The CMD codes are defined in susfs_def.h
 * and must match the values used by the ksu_susfs userspace tool.
 */

#ifdef CONFIG_KSU_SUSFS_SUS_MAP
static int cmd_add_sus_maps(unsigned long arg)
{
	return susfs_add_sus_maps((struct st_susfs_sus_maps __user *)arg);
}

static int cmd_update_sus_maps(unsigned long arg)
{
	return susfs_update_sus_maps((struct st_susfs_sus_maps __user *)arg);
}
#endif

#ifdef CONFIG_KSU_SUSFS_SUS_PROC_FD_LINK
static int cmd_add_sus_proc_fd_link(unsigned long arg)
{
	return susfs_add_sus_proc_fd_link((struct st_susfs_sus_proc_fd_link __user *)arg);
}
#endif

#ifdef CONFIG_KSU_SUSFS_SUS_MEMFD
static int cmd_add_sus_memfd(unsigned long arg)
{
	return susfs_add_sus_memfd((struct st_susfs_sus_memfd __user *)arg);
}
#endif

...
/* MUST match build/patches/susfs_v2/susfs_gki_backport.c struct st_susfs_sus_maps
   byte-for-byte (aarch64, SUSFS_MAX_LEN_PATHNAME=256). Copied verbatim so the kernel's
   copy_from_user(sizeof(struct st_susfs_sus_maps)) lands on the right fields. */
...
static void print_help(void) {
	...
	log("\n");
	log("        add_sus_maps </target/path> </spoofed/path>\n");
	log("         |--> Sanitize the shown pathname of <target/path> in /proc/pid/maps (SUS_MAP backport)\n");
	log("\n");
	log("        update_sus_maps </target/path>\n");
	log("         |--> Update the target_ino for an existing SUS_MAPS entry\n");
	log("\n");

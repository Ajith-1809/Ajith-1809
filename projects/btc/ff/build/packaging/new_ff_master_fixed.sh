...
"
  ...
  if [ -n "$FF_UID" ]; then
    for P in $FF_PATHS; do
      ksu_susfs add_sus_path_for_uid \
      $FF_UID "$P" 2>/dev/null
    done
    echo "FF MAX UID done!" >> $LOG
  fi

  if [ -n "$FF2_UID" ]; then
    for P in $FF_PATHS; do
      ksu_susfs add_sus_path_for_uid \
      $FF2_UID "$P" 2>/dev/null
    done
    echo "FF Normal UID done!" >> $LOG
  fi

  if [ -n "$GMS_UID" ]; then
    for P in $GMS_PATHS; do
      ksu_susfs add_sus_path_for_uid \
      $GMS_UID "$P" 2>/dev/null
    done
    echo "GMS UID done!" >> $LOG
  fi

  if [ -n "$PLAY_UID" ]; then
    for P in $GMS_PATHS; do
      ksu_susfs add_sus_path_for_uid \
      $PLAY_UID "$P" 2>/dev/null
    done
    echo "Play UID done!" >> $LOG
  fi

  if [ -n "$GSF_UID" ] && \
     [ "$GSF_UID" != "$GMS_UID" ]; then
    for P in $GMS_PATHS; do
      ksu_susfs add_sus_path_for_uid \
      $GSF_UID "$P" 2>/dev/null
    done
    echo "GSF UID done!" >> $LOG
  fi
...
process_lib() {
  ...
  "$SRCPATH" 2>/dev/null
  ksu_susfs update_sus_kstat_full_clone \
  "$SRCPATH" 2>/dev/null
...
while [ -z "$FF2PATH" ] && \
  ...
  if [ -z "$FF2PATH" ]; then
    RETRY=$((RETRY+1))
    echo "Retrying FF Normal... ($RETRY/15)" \
    >> $LOG
    sleep 2
  fi
done

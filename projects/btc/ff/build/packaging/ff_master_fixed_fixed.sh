...
# ============================================
# LOCK â€” Prevent duplicate instances
# ============================================
LOCKFILE="/data/local/tmp/ff_master.lock"

# Atomic lock â€” noclobber fails if file already exists,
...
( set -o noclobber
  ...
  # Stale lock â€” take it over atomically
  ( set -o noclobber
    echo $$ > "$LOCKFILE"
  ) 2>/dev/null || exit 0
}
...
"
  ...
  # so we use global add_sus_path instead â€” hides from all apps.
...
# Auto generated â€” no global hiding
EOF
...
# Auto generated â€” root paths only
/data/adb/ksu
/data/adb/ksu/bin
/data/adb/susfs4ksu
/debug_ramdisk
...
# ============================================
# FUNCTION â€” AUTO BACKUP LIBS
#
# 32-bit:
#   FFPATH (arm)  â†’ BACKUP2
#   FF_LIB        â†’ BACKUP2
#   FF2PATH (arm) â†’ BACKUP2
#   FF_LIB2       â†’ BACKUP2
#
# 64-bit:
#   FFPATH (arm64)  â†’ BACKUP
#   FF2PATH (arm64) â†’ BACKUP
# ============================================
...
# ============================================
# FUNCTION â€” APPLY ALL REDIRECTS + KSTAT
#
...
apply_redirects_and_kstat() {
  ...
  if [ "$IS_32BIT" = "1" ]; then
    ...
    # FF MAX extracted lib â†’ BACKUP2
    ...
    # FF Normal extracted lib â†’ BACKUP2
    if [ -d "$FF_LIB2" ]; then
...
# ============================================
# STEP 3 â€” UPDATE SUSFS FILES
# ============================================

update_susfs_files

# ============================================
# STEP 4 â€” LIVE APPLY UID HIDING
# ============================================

apply_uid_hiding

# ============================================
# STEP 5 â€” FIND GAME PATHS
#
...
# FF Normal path â€” match same arch as FF MAX
FF2PATH=""
if [ "$IS_32BIT" = "1" ]; then
...
# ============================================
# STEP 6 â€” AUTO BACKUP LIBS
# ============================================

auto_backup_libs

# ============================================
# STEP 7 â€” APPLY REDIRECTS + KSTAT + PROC
# ============================================

apply_redirects_and_kstat

# ============================================
# STEP 8 â€” ANDROID ID + CACHE CLEAR
# ============================================
